...
import {createTypedArray,getNumberInRange,lim,rand,findGridCell,P} from "../utils/index.js"

import {grid,seed,template as templateInput} from "../main.js"

export const HeightmapGenerator = (function () {
  let grid = null;
  let heights = null;
  let blobPower;
  let linePower;
  ...
  const addHill = (count, height, rangeX, rangeY) => {
    ...
    function addOneHill() {
      ...
      do {
        const x = getPointInRange(rangeX, grid.w);
        const y = getPointInRange(rangeY, grid.h);
  ...
  const addRange = (count, height, rangeX, rangeY, startCell, endCell) => {
    ...
    function addOneRange() {
      ...
      if (rangeX && rangeY) {
        ...
        const startX = getPointInRange(rangeX, grid.w);
        const startY = getPointInRange(rangeY, grid.h);
        ...
        do {
          endX = Math.random() * grid.w * 0.8 + grid.w * 0.1;
          endY = Math.random() * grid.h * 0.7 + grid.h * 0.15;
          dist = Math.abs(endY - startY) + Math.abs(endX - startX);
          limit++;
        } while ((dist < grid.w / 8 || dist > grid.w / 3) && limit < 50);
    ...
    function addOneTrough() {
      ...
      if (rangeX && rangeY) {
        ...
        let limit = 0,
          ...
          endY;
        do {
          startX = getPointInRange(rangeX, grid.w);
          startY = getPointInRange(rangeY, grid.h);
          startCell = findGridCell(startX, startY, grid);
          limit++;
        ...
        do {
          ...
          dist = Math.abs(endY - startY) + Math.abs(endX - startX);
          limit++;
        } while ((dist < grid.w / 8 || dist > grid.w / 2) && limit < 50);
  ...
  const addStrait = (width, direction = "vertical") => {
    ...
    const startX = vert ? Math.floor(Math.random() * grid.w * 0.4 + grid.w * 0.3) : 5;
    const startY = vert ? 5 : Math.floor(Math.random() * grid.h * 0.4 + grid.h * 0.3);
    const endX = vert
      ? Math.floor(grid.w - startX - grid.w * 0.1 + Math.random() * grid.w * 0.2)
      : grid.w - 5;
    const endY = vert
      ? grid.h - 5
      : Math.floor(grid.h - startY - grid.h * 0.1 + Math.random() * grid.h * 0.2);
  ...
  const mask = (power = 1) => {
    ...
    heights = heights.map((h, i) => {
      ...
      const nx = (2 * x) / grid.w - 1; // [-1, 1], 0 is center
      const ny = (2 * y) / grid.h - 1; // [-1, 1], 0 is center
