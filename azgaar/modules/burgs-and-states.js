...
import {P,rn,rw,ra,rand,gauss,minmax,getColors,getRandomColor,getMixedColor,getAdjective,trimVowels,each,getMiddlePoint} from "../utils/index.js"

import {Names} from "./names-generator.js"
import {COA} from "./coa-generator.js"
import {Routes} from "./routes-generator.js"

import {seed,pack,grid,biomesData,power,year,regions as nRegions,manors as manorsInput,provinces as provincesInput,neutralRate} from "../main.js"
...
export const BurgsAndStates = (() => {
  const generate = () => {
    ...
    function placeCapitals() {
      ...
      let spacing = (grid.w + grid.h) / 2 / count; // min distance between capitals
    ...
    function createStates() {
      ...
      burgs.forEach((b, i) => {
        ...
        // states data
        const expansionism = rn(Math.random() * power + 1, 1);
    ...
    function placeTowns() {
      ...
      let spacing = (grid.w + grid.h) / 150 / (burgsNumber ** 0.7 / 66); // min distance between towns
  ...
  const getType = (i, port) => {
    ...
    let _pop = pack.burgs[cells.burg[i]].population
    if (!cells.burg[i] || _pop < 6) {
      if (_pop < 5 && [1, 2, 3, 4].includes(cells.biome[i])) return "Nomadic";
  ...
  const expandStates = () => {
    ...
    const globalNeutralRate = neutralRate;
    const statesNeutralRate = 1;
  ...
  const generateProvinces = (regenerate = false, regenerateInLockedStates = false) => {
    ...
    const percentage = provincesInput;
    ...
    //get neighboring
    provinces.forEach(p => {
      if (p.removed || p==0) return;
      p.neighbors = new Set();
    });

    // check for neighboring province
    for (const i of cells.i) {
      let p = cells.province[i]
      if (cells.h[i] < 20 || p == 0) continue;
        
      cells.c[i]
        .filter(c => cells.h[c] >= 20 && cells.province[c] !== p)
        .forEach(c => provinces[p].neighbors.add(cells.province[c]));
    }

    // convert neighbors Set object into array
    provinces.forEach(p => {
      if (!p.neighbors) return;
      p.neighbors = Array.from(p.neighbors);
    });
