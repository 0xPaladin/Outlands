import {round, each, last, rn, rw} from "../utils/index.js"

import {Names,Lakes,seed,points,pack,grid,resolveDepressions as resolveDepressionsStepsOutput} from "../main.js"

// default color scheme
const lineGen = d3.line().curve(d3.curveBasis);

export const Rivers = (function () {
  const generate = function (allowErosion = true) {
    ...
    function defineRivers() {
      ...
      const defaultWidthFactor = rn(1 / (points / 10000) ** 0.25, 2);
  ...
  const getBorderPoint = i => {
    ...
    const min = Math.min(y, grid.h - y, x, grid.w - x);
    if (min === y) return [x, 0];
    else if (min === grid.h - y) return [x, grid.h];
