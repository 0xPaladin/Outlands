...
import "../libs/delaunator.min.js"
import {rn,createTypedArray} from "./index.js"
...
export function generateGrid(seed,n=10000,w=1000,h=1000) {
  Math.random = aleaPRNG(seed); // reset PRNG
  const {spacing, cellsDesired, boundary, points, cellsX, cellsY} = placePoints(n,w,h);
  const {cells, vertices} = calculateVoronoi(points, boundary);
  return {spacing, cellsDesired, boundary, points, cellsX, cellsY, cells, vertices, seed,w,h};
}

// place random points to calculate Voronoi diagram
export function placePoints(n,w,h) {
  console.time("placePoints");
  const cellsDesired = n;
  const spacing = rn(Math.sqrt((w * h) / cellsDesired), 2); // spacing between points before jirrering

  const boundary = getBoundaryPoints(w, h, spacing);
  const points = getJitteredGrid(w, h, spacing); // points of jittered square grid
